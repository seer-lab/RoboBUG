...
public class Menu : MonoBehaviour
{
	...
	float delaytime = 0f;
	float delay = 0.1f;
		string lfile = @"leveldata\levels.txt";
		...
		if (!gameon) {
			...
			if (Input.GetKeyDown (KeyCode.Return) && delaytime < Time.time) {
								switch (option) {
								case 0:
										lg.BuildLevel (@"leveldata\" + levels [0], false);
										lg.gamestate = 2;
										gameon = true;
										buttons [4].GetComponent<SpriteRenderer> ().color = Color.white;
										break;
			...
			} else{				m2arrows[1].GetComponent<SpriteRenderer>().enabled = false;
			}
			if (levoption != 0) {
				m2arrows[0].GetComponent<SpriteRenderer>().enabled = true;
			} else{				m2arrows[0].GetComponent<SpriteRenderer>().enabled = false;
			}
						...
						if (Input.GetKeyDown (KeyCode.Return)) {
								switch (option) {
								case 0:
										lg.BuildLevel (@"leveldata\" + levels [levoption], false);
										lg.gamestate = 2;
										gameon = true;
										buttons [4].GetComponent<SpriteRenderer> ().color = Color.white;
										m2switch (false);
		...
		public void saveGame (string currentlevel)
		{
				...
				StreamWriter sw = File.CreateText (@"leveldata\levels.txt");
